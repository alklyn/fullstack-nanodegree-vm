...
def restaurant_menu_json(restaurant_id):
    ...
    menu_items = \
        db_session.query(MenuItem).filter_by(restaurant_id=restaurant.id).all()
...
def restaurants():
    ...
    """
    restaurants = db_session.query(Restaurant).order_by(Restaurant.id).all()
...
def edit_restaurant(restaurant_id):
    ...
    if "username" not in session:
        return redirect("/login/")
    elif session["user_id"] != restaurant.user_id:
    ...
    if request.method == "POST":
        ...
        restaurant = \
            db_session.query(Restaurant).filter_by(id=restaurant_id).one()
...
def show_menu(restaurant_id=-1):
    ...
    for course in courses:
        ...
        menus.append(db_session.query(MenuItem)\
...
def edit_menu_item(restaurant_id, menu_id):
    ...
    if request.method == "POST":
        if request.form["choice"] == "edit":
            menu_item = \
                db_session.query(MenuItem).filter_by(id=menu_id).first()
